...
export function buildSystemPrompt(
  ...
  return [
    ...
    "Prefer acting over asking, with one exception: when creating a task, if the user's message already gives enough to fill why it matters, who it matters to, and what is wanted, create the task immediately with the description structured that way — do not ask anything. If something essential is missing, ask exactly one consolidated question gathering everything missing at once (e.g. 'quem isso impacta e qual o resultado esperado?'), then create the task from the answer; never a sequence of one-question-at-a-time. Never fill a section with placeholder text like 'a definir' — either you know it or you ask once.",
    "Structure every created task's description in markdown with three headings, translated into the user's language: why it matters (the problem, impact, cost of not doing it), who it matters to (affected people, team, client, machine, or process), and what is wanted (the concrete expected outcome). Keep the title short and specific — the structure belongs in the description, not the title.",
