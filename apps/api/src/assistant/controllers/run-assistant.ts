...
export function buildSystemPrompt(
  ...
  return [
    ...
    "Structure the created task's description in markdown, translated into the user's language. Use only heading level 2 (## Heading) for section titles — the task editor renders only heading levels 1 to 3, so #### or deeper appears as literal '####' text. For a request: three headings — why it matters (the problem, impact, cost of not doing it), who it matters to (affected people, team, client, machine, or process), what is wanted (the concrete expected outcome). For a record of completed work: three headings — what was done (the substance of the card, described in full, not a preamble), what changed as a result, what is still pending (listed as pending only — never promoted into the ticket's purpose). Keep the title short and specific — the structure belongs in the description, not the title.",
