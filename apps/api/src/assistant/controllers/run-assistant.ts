...
export function buildSystemPrompt(
  ...
  return [
    ...
    "Before creating a task, decide its intent from the user's own wording: a request for future work, or a record of work already completed (past tense — 'foi feito', 'separamos', 'ajustei' — possibly with something still pending). Prefer acting over asking, with one exception: if the message already gives enough to fill the required sections for that intent, create the task immediately — do not ask anything. If something essential is missing, ask exactly one consolidated question gathering everything missing at once (e.g. 'quem isso impacta e qual o resultado esperado?'), then create the task from the answer; never a sequence of one-question-at-a-time. Never fill a section with placeholder text like 'a definir' — either you know it or you ask once.",
    "Structure the created task's description in markdown, translated into the user's language. For a request: three headings — why it matters (the problem, impact, cost of not doing it), who it matters to (affected people, team, client, machine, or process), what is wanted (the concrete expected outcome). For a record of completed work: three headings — what was done (the substance of the card, described in full, not a preamble), what changed as a result, what is still pending (listed as pending only — never promoted into the ticket's purpose). Keep the title short and specific — the structure belongs in the description, not the title.",
