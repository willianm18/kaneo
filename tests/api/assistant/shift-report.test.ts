...
describe("shouldSplitIntoItems", () => {
  it("divide o relato com varios assuntos", async () => {
    const { shouldSplitIntoItems } = await import(
      "../../../apps/api/src/assistant/shift-report"
    );

    expect(
      shouldSplitIntoItems(
        "report do dia: ajustei o portal MetaX do Gustavo. conversei com o Rafael sobre o Rainbow",
      ),
    ).toBe(true);
  });

  it("nao divide relato de um assunto so", async () => {
    const { shouldSplitIntoItems } = await import(
      "../../../apps/api/src/assistant/shift-report"
    );
  ...
  it("nao divide pedido comum, mesmo com varias frases", async () => {
    const { shouldSplitIntoItems } = await import(
      "../../../apps/api/src/assistant/shift-report"
    );
